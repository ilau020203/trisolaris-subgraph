...
// export function handleSwap(event: SwapEvent): Swap {
...
//   swap.sender = event.params.sender.toHex().toLowerCase()
...
// //   const pair = getPair(event.address.toHex().toLowerCase())
// //   const snapshot = getAprSnapshot(event.address.toHex().toLowerCase(), event.block.timestamp)
